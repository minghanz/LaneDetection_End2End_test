...
def instersection_points(params, M_inv, resize):
    params0 = *(params[0]), 'left'
    params1 = *(params[1]), 'right'
    params2 = *(params[2]), 'left'
    params3 = *(params[3]), 'right'
    x,y,order = [],[],[]
    ...
    x = np.array(x)
    y = np.array(y)
    x_prime, y_prime = homogenous_transformation(M_inv,x,y)
    maxima = np.zeros(4)
    maxima.fill(resize-1)
    for i, idx in enumerate(order):
        maxima[idx] = y_prime[i]*(resize-1)
    return maxima
...
def draw_fitted_line(img, params, y_prime):
    # minimum, maximum = np.min(y_vals), np.max(y_vals)
    a, b, c = [*params]
    x_pred = a*(y_prime)**2 + b*(y_prime) + c    
    return img, x_pred


def find_closest_lanes(data_lanes):
    lst = []
    for lane in data_lanes:
        for i, point in enumerate(reversed(lane)):
            if point != -2:
                lst.append(i)
                break
    lowest = sorted(range(len(lst)), key=lambda x: lst[x])[0:2]
    lowest = [x+1 for x in lowest]
    return tuple(sorted(lowest))
